...
def train(filepath,codec,savepath,sg=0,epochs=56,size=300,min_count=10,window=5,worker=4, hs=0, negative=5, ns_exponent=0.75,sample=1e-3):
    print("strat")
    now = datetime.now()
    ...
    model = gensim.models.Word2Vec(sentences, sg=sg,iter=epochs,size=size,min_count=min_count,window=window,workers=worker,hs=hs,negative=negative,ns_exponent=ns_exponent,sample=sample)
    print("end")
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    print("Current Time =", current_time)
